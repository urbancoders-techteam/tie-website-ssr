...
export default function WhoCanApply() {
  ...
  return (
    <section className="overflow-x-hidden bg-[#f7f9fc] py-12 sm:py-14 lg:py-20">
      <div className="mx-auto w-full max-w-7xl min-w-0 px-4 sm:px-6 lg:px-8">
        ...
        {/* 2 cols tablet (768–1023); 3 cols laptop+ so each card gets enough width */}
        <div className="mx-auto mt-10 grid w-full min-w-0 grid-cols-1 gap-6 sm:mt-12 md:grid-cols-2 md:gap-5 lg:mt-14 lg:grid-cols-3 lg:gap-5 xl:gap-6">
          {cards.map((card) => (
            <article
              key={card.id}
              className="flex min-w-0 flex-col overflow-hidden rounded-3xl border border-gray-100/90 bg-white shadow-[0_16px_48px_rgba(7,27,58,0.08)]"
            >
              {/* Fixed hero height on laptop 3-col so all three cards fit the viewport better */}
